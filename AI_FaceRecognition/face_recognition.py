...
class Face_Recognition:
    def __init__(self, root):
        ...
        #1st image
        img_top = Image.open(r"project_images/image bg.jpeg")
        img_top = img_top.resize((700,650), Image.Resampling.LANCZOS)
        self.photoimg1 = ImageTk.PhotoImage(img_top)

        f_lbl = Label(self.root, image=self.photoimg1)
        f_lbl.place(x=10, y=50,  width=600, height=650)

        #2st image
        img_bottom=Image.open(r"project_images/face detector.jpeg")
        img_bottom=img_bottom.resize((700,650),Image.Resampling.LANCZOS)
        self.photoimg_bottom=ImageTk.PhotoImage(img_bottom)

        f_lbl=Label(self.root,image=self.photoimg_bottom)
        f_lbl.place(x=650,y=50, width=600, height=650)
    ...
    def face_recog(self):
        def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in features:
                ...
                if confidence>77 :
                    cv2.putText(img,f"ID:{i}",(x,y-75),cv2.FONT_HERSHEY_COMPLEX,0.8,(240, 248, 255),3)
                    cv2.putText(img,f"Roll:{r}",(x,y-55),cv2.FONT_HERSHEY_COMPLEX,0.8,(240, 248, 255),3)
                    cv2.putText(img,f"Name:{n}",(x,y-30),cv2.FONT_HERSHEY_COMPLEX,0.8,(240, 248, 255),3)
                    cv2.putText(img,f"Department:{d}",(x,y-5),cv2.FONT_HERSHEY_COMPLEX,0.8,(240, 248, 255),3)
            ...
            return coord
                    ...
                    #    ITNE PART ME KUCH ERROR HAI ISKE BINA SHI CHAL RHA 
